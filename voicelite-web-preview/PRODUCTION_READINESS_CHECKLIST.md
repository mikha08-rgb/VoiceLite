...
- [ ] Check signing key environment variables are configured
  - `VOICELITE_LICENSE_PUBLIC_KEY` matches production `LICENSE_SIGNING_PUBLIC_B64`
  - `VOICELITE_CRL_PUBLIC_KEY` matches `CRL_SIGNING_PUBLIC_B64`
  - Private keys are stored only in the deployment secret manager

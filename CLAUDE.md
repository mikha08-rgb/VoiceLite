...
- **Uninstall preserves user data by default; pass `/PURGEDATA` to wipe**: `VoiceLiteSetup.iss` data-preservation policy lives in two places that must agree: (1) `[UninstallDelete]` deliberately does NOT list `{localappdata}\VoiceLite` (only `{app}\temp`), and (2) `CurUninstallStepChanged` checks `PurgeDataRequested` (the `/PURGEDATA` command-line flag) before wiping. Both must hold — listing `{localappdata}\VoiceLite` in `[UninstallDelete]` would make the policy a no-op because `[UninstallDelete]` runs during the file-deletion phase, BEFORE `usPostUninstall` fires the code-event check. This was the v2.1.1 ship-bug: the code path was right but `[UninstallDelete]` still wiped the dir first. Fixed in v2.1.2 by removing the entry. Rules: silent uninstall without `/PURGEDATA` preserves data (Inno upgrades, Intune redeploys); interactive uninstall asks via clearly-worded dialog with NO as default; `unins000.exe /VERYSILENT /PURGEDATA` is the explicit decommission path for IT admins.
- **`AutoPaste` setting controls clipboard injection, not just Ctrl+V**: `MainWindow.xaml.cs:1434` branches on `settings.AutoPaste`. When true, `TextInjector.InjectText` writes clipboard + sends Ctrl+V (2s auto-clear). When false, `TextInjector.CopyToClipboardForManualPaste` writes clipboard only with a 30s hold — long enough for a clinician to click into their target field before pasting manually. The setting name is misleading historically (sounds like "auto-paste vs no paste") but actually picks between two paths that both involve the clipboard. v2.1.1 had this as `if (AutoPaste) { inject } // else nothing` — text just disappeared into history, breaking the manual-paste workflow GPHealth needed. Fixed in v2.1.2.
